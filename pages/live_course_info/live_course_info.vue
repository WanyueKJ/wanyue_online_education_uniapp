<template>
	<view class="app">
		...
		<!-- #ifndef MP-WEIXIN -->
		<uni-nav-bar @clickLeft="backCourseList" left-icon="back" :border="false" title="直播详情">
				...
				<swiper-item class="course-info-item">
					<scroll-view scroll-y :style="'height:' + scrollH+'px;'">
						...
						<view class="price-time-stunum">
							...
							<text class="time-wrap">时间: {{liveInfo.lesson}}</text>
		...
		methods: {
			...
			enterlive() {
				...
				if (liveInfo.type == 7) {
					...
					// });
					uni.navigateTo({
						url: '../live-info/live-info?liveuid=' + liveInfo.uid +
							'&courseid=' + liveInfo.id + '&lessonid=' + '0' + '&thumb=' + this.live_course_bg,
					});
				}
			},
